...
private:
    ...
    friend ostream& operator<<(ostream& ostr, Body& body){
        ostr << "Name: " << body.name << " Orbit: " << body.orbit << " Mass: " << body.mass << " Position: "
             << body.pos <<", Velocity: " << body.v << ", Acceleration: " << body.a << endl;
        return ostr;
    }
...
public:
    ...
    SolarSystem(string solardata) {
        ...
        getline(solarfile, s);
        string tname, torbit;
        double tmass, tdiam, tperh, taphe, skip1, skip2, skip3, skip4, ang, r;
        Vec3d tpos, tv, ta;
        int xrand, yrand, zrand;
        double x, y, z, vel;
        double sunMass;
        while(solarfile >> tname >> torbit >> tmass >> tdiam >> tperh >> taphe >> skip1 >> skip2 >> skip3 >> skip4){
            //solarfile >> tname >> torbit >> tmass >> tdiam >> tperh >> taphe >> skip1 >> skip2 >> skip3 >> skip4;
            if (tname == "Sun"){
                sunMass = tmass;
            }
            if (tname == "Earth" || tname == "Jupiter" || tname == "Mars" || tname == "Moon" || tname == "Sun" || tname == "Io" || tname == "Europa" || tname == "Gaynmede" || tname == "Callisto" || tname == "Phobos" || tname == "Deimos")
            {
                continue;
            }
            xrand = rand() % 10 +1;
            ...
            cout << "Planet name: " << tname << " Orbit name: " << torbit << " Mass: " << tmass << " Position: " << tpos <<
